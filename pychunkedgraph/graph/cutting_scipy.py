...
# def merge_cross_chunk_edges_scipy(
#     edges: Iterable[Sequence[np.uint64]], affs: Sequence[np.uint64]
# ):
#     """Merges cross chunk edges
#     :param edges: n x 2 array of uint64s
...
class LocalMincutGraphScipy:
    ...
    def __init__(
        ...
        negative_affs = mapped_affs < 0
        mapped_affs = mapped_affs.astype(np.int64)
        new_negative_affs = mapped_affs < 0 & ~negative_affs
        mapped_affs[new_negative_affs] = np.iinfo(np.int64).max
        ...
        self._build_scipy_graph(mapped_edges, mapped_affs)
    ...
    def _build_scipy_graph(self, edges, affs):
        """
        ...
        (
            self.sparse_mat_raw,
            self.scipy_edges_raw,
            _,
        ) = flatgraph.build_scipy_sparse_mat(edges, affs, make_directed=True)
        ...
        ) = flatgraph.build_scipy_sparse_mat(comb_edges, comb_affs, make_directed=True)
...
def run_multicut(
    ...
    *,
    ...
    local_mincut_graph = LocalMincutGraphScipy(
        edges.get_pairs(),
        edges.affinities,
        source_ids,
        sink_ids,
